...
import { handleSeedSettingAPI, GenerateSeed } from "$lib/gameAPIs/dice"
import { handleAuthToken } from "$lib/store/routes"
import { DiceEncription } from '$lib/games/ClassicDice/store/index';
export let settin;
$: settings = settin || $DiceEncription
...
onMount(async()=>{
    loading = true
    const { load, response } = await GenerateSeed()
    if(response){
        client = response?.clientSeed
        server = response?.serverSeed
        hash = response?.hash
    }
    loading = load
})
...
const regex = /^[a-zA-Z0-9]+$/;
    ...
    if(!client || client.length < 10){
       ...
       const res  = await handleSeedSettingAPI($handleAuthToken, {client, server, hash})
       if(res?.response){
            handleCancle()
       }
       is_loading = res?.loading
    }
})
            ...
            <div class="detailForm">
                ...
                <div class="sc-ezbkAF kDuLvp input ">
                    ...
                    <div class="input-control">
                        <input type="text" readonly value={settings?.server_seed}>
                    </div>
                </div>
                <div class="formFlex">
                    <div class="sc-ezbkAF kDuLvp input ">
                        ...
                        <div class="input-control">
                            <input type="text" readonly value={settings?.client_seed}>
                        </div>
                    </div>
                    <div class="sc-ezbkAF kDuLvp input ">
                        <div class="input-label">Nonce</div>
                        <div class="input-control">
                            <input type="text" readonly value={settings?.game_nonce || settings?.nonce}>
                        </div>
                    </div>
                </div>
            </div>
