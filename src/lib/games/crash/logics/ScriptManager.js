...
export default class ScriptManager extends BaseScriptManager {
  ...
  constructor(game) {
    ...
    api_script.subscribe(item =>{
      this.setBackendUrl(item.backend_url)
    })
  }

  setBackendUrl(item){
    this.backend_url = item
  }

  async initScript() {
    if (!this.initPms) {
      this.initPms = axios
        .post(this.backend_url + "/api/user/crash-game/scripts/list/", {
  ...
  async delScript({ id }) {
    await axios.post(this.backend_url + "/api/user/crash-game/scripts/delete/", {
      id,
    }, {
      headers: {
  ...
  async updateScript(script) {
    ...
    if (id == 0) {
      ...
      } = await axios.post(this.backend_url + "/api/user/crash-game/scripts/add/", {
        gameName: this.game.name,
        name,
        content,
      }, {
      ...
      await axios.post(this.backend_url + "/api/user/crash-game/scripts/update/", {
        id,
        name,
        content,
      }, {
